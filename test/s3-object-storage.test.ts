...
describe("S3-compatible object storage", (): void => {
  it("signs the complete multipart lifecycle against a path-style endpoint", async (): Promise<void> => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      ...
      if (request.method === "POST" && url.search === "?uploads") return new Response("<InitiateMultipartUploadResult><UploadId>provider-upload-1</UploadId></InitiateMultipartUploadResult>");
      if (request.method === "PUT" && url.searchParams.has("partNumber")) return new Response(null, { headers: { etag: '"part-etag"' } });
      if (request.method === "POST" && url.searchParams.has("uploadId")) return new Response("<CompleteMultipartUploadResult><ETag>&quot;final-etag&quot;</ETag></CompleteMultipartUploadResult>");
      if (request.method === "GET") return new Response("stored bytes", { headers: { "content-length": "12", "content-type": "text/plain", etag: '"final-etag"' } });
    ...
    expect(await storage.uploadPart(key, started.uploadId, 1, new Blob([chunk]).stream(), chunk.byteLength)).toEqual({ etag: '"part-etag"', partNumber: 1 });
    expect(await storage.completeMultipart(key, started.uploadId, [{ etag: '"part-etag"', partNumber: 1 }])).toEqual({ etag: '"final-etag"', size: null });
  ...
  it("caps error-body reads before reporting an upstream failure", async (): Promise<void> => {
    vi.stubGlobal("fetch", vi.fn(async (): Promise<Response> => new Response("x".repeat(10_000), { status: 500 })));
    await expect(makeS3ObjectStorage(config).delete("user-1/file.txt")).rejects.toThrow(/x{2000}$/);
  });
});
