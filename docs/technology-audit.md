...
| --- | --- | --- | --- |
| Full-stack framework | TanStack Start 1.168.44 | SSR, streaming, typed file routing, loaders, and Worker-native server routes without an adapter layer | React Router framework mode for a broader ecosystem; Hono plus Vite for a thinner API/SPA architecture |
| Routing | TanStack Router 1.170.27 | End-to-end route, search, loader, and navigation types | Bundled with the framework decision; do not replace independently |
...
| Worker integration | Cloudflare Vite plugin 1.52.0 | Direct Workers development/build integration and binding access | Wrangler-only custom Worker entrypoint for a non-framework application |
| Cloudflare CLI | Wrangler 4.122.0 | Type generation, local bindings, D1 migrations, dry-runs, and deployment | Keep; it is the platform control plane even if the UI framework changes |
| Authentication | Better Auth 1.6.27 + D1 adapter | Application-owned users, sessions, rate limits, and email/password auth | Cloudflare Access for an internal workforce app; Auth.js or a hosted identity vendor if ownership/operations priorities reverse |
...
| Testing | Vitest 4.1.10 + Workers pool 0.21.2 | D1/R2/Better Auth/Effect integration plus signed generic-S3 contract tests in `workerd` | Keep; Node mocks would provide weaker platform evidence |
...
- The Worker compatibility date is 2026-08-12 and the custom server entry attaches the production response security policy directly to dynamic SSR responses.
...
- Cloudflare is the sole deployment target: `drivebox` binds the `drivebox` D1 database and `drivebox-files` R2 bucket. Vercel Git deployments are explicitly disabled in `vercel.json`.
...
The August 12, 2026 release also passed live production signup, session, multipart upload, byte-for-byte download, delete, responsive-browser, response-header, and QA-data cleanup checks on the deployed Worker.
